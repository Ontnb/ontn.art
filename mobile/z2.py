...
# 2. Поместите этот скрипт в папку "mobile".
# 3. В корневой папке создайте папку с медиафайлами (например, "2").
# 4. В этой папке должны находиться фото (.jpg, .webp) и видео (.mp4, .webm).
# 5. Для каждого видео должно быть фото с тем же именем, что и у видео (используется как превью).
# 6. Запустите скрипт. Он создаст HTML-файл с именем, соответствующим имени папки с медиафайлами.
# 7. HTML-файл будет сохранен в папке "mobile".

# Укажите имя папки с медиафайлами (например, "2")
media_folder_name = "6"
...
if not media_folder_path.exists():
    raise FileNotFoundError(f"Папка с медиафайлами '{media_folder_path}' не найдена!")
    ...
    return f"""
            <div class="portfolio-item">
                <div class="video-container">
                    <video class="portfolio-video" preload="none" data-src="../{media_folder_name}/{video_filename}" data-poster="../{media_folder_name}/{poster_filename}" playsinline webkit-playsinline>
...
def create_photo_html(photo_filename):
    return f"""
            <div class="portfolio-item">
                <img data-src="../{media_folder_name}/{photo_filename}" alt="{photo_filename}">
